...
@jwt_required
def audio_video_transcribe(media_id):
    ...
    if (db_write("""UPDATE media_datas SET transcribe_status=%s, transcribe_url=%s, transcribe_s3_url=%s, updatedAt=%s WHERE id=%s""",
        ...
        presigned_url = s3.generate_presigned_url('get_object', Params={'Bucket': S3_TRANSCRIBE_BUCKET, 'Key': s3_job_name},
                                              ExpiresIn=259200) # expire 3 * 24 * 3600
        print("presigned_url", presigned_url)
        return jsonify({"jwt_token": refresh_token(), "msg": "success", "success": "true", "transcribe_url": presigned_url, "mediaId": media_id, "index": index, "file_name": file_name[0]['file_name']}), 201
    else:
